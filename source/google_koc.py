...
def main():
    ...
    refresh_token = '1//053WRMZLEdomfCgYIARAAGAUSNwF-L9Ir20tXBgR0__MpfikenOB109LIeYtMCCbo6u9Q7WziS77t1O81uzwcdlkoz9X2vbmNcqw'
    developer_token = 'PJY5mhXidEqXfbuCZES9PQ'
    client_customer_id = '403-999-3356'
    ...
    geo_camp_total = geo_camp_total.groupby(['Day', 'locationName']).sum().reset_index().sort_values('Day', ascending = True)
    ...
    fig = make_subplots( rows=2, cols=2, column_widths=[0.5, 0.5], specs=[[{'secondary_y':True}, {'secondary_y':True}],[{ "colspan": 2, "secondary_y": True}, None]],
                    subplot_titles=("Daily Adwords Conversions vs. CPC", "Daily Adwords Revenue KPIs", "User KPIs" ))

    # Add traces

    fig.add_trace(go.Scatter(x=uac_plot.Day, y=uac_plot['Cost'],
                    mode='lines+markers',
                    name='Cost', marker_color = '#109618'), row =1 , col = 1)
    fig.add_trace(go.Scatter(x=uac_plot.Day, y=uac_plot.avgCPC, name = 'CPC', marker_color = '#EF553B'), secondary_y = True, row=1, col=1)
    fig.add_trace(go.Bar(x=plot_rev.groupby('Day').sum().index, y=plot_rev.groupby('Day').sum()['Total conv. value'], marker_color = '#FEAF16', name = 'Conv. Value', showlegend = False), row=1, col=2)
    ...
    fig.add_trace(go.Scatter(x = uac_plot['Day'], y = uac_plot['Total conv. value'] / uac_plot['Conversions'], marker_color = 'purple', name = 'ARPU', mode="lines+markers"), secondary_y= True, row =2, col = 1)
    fig.add_trace(go.Scatter(x = uac_plot['Day'], y = uac_plot['Cost'], marker_color = '#109618', name = 'Cost', mode="lines+markers", showlegend = False),  row =2, col = 1)
    fig.add_trace(go.Scatter(x=uac_plot['Day'], y=uac_plot['Conversions'],
                    name='Conversions', marker_color = 'cyan', showlegend = False), row=2, col=1)
    ...
    fig.update_layout(barmode = 'stack', height = 1000, title_text = 'OVERVIEW')
    ...
    fig_camp2 = px.scatter(geo_camp_total, x='Day', y='Total conv. value', facet_col='locationName', facet_col_wrap=2, size = 'Conversions').update_traces(mode='lines+markers')
    ...
    fig_camp2.update_layout(
        title_text="Daily Revenue per Country", height = 550)
    
    figs_gads.append(fig_camp2)
    ...
    def fbudgets(days, df):
        ...
        while days >= 1:
            ...
            df_1 = pd.DataFrame(pd.concat([temp,df[df['Day'] == (datetime.now() - timedelta(days= 15 - days)).strftime('%b-%d')]['Budget Name'], df[df['Day'] == (datetime.now() - timedelta(days= 15 - days)).strftime('%b-%d')]['ROAS']], axis = 1))
    ...
    tab.update_layout(title_text = "Budget Stats Current Week (Green Rows above 1 Today's ROAS)", height = 400)
    ...
    with open('Adwords_koc_daily.html', 'w') as f:
