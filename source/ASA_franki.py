...
access_token = "eyJhbGciOiJkaXIiLCJlbmMiOiJBMjU2R0NNIiwia2lkIjpudWxsfQ..nbMdp_ofGGnozKpd.n6Zt267dCfaBcWjEjzYAScliLp9y26SzAW1PSIwTXzYtdGpcK_fdoz12ePpwONtaaea8jbSAroj-caSRx2OlG9LN7mPPP0A8do_JQtl8FIxHWAp6efopA47nSUEDygATGxDaWmG9fE_M-SPuAyR_XApzcmsqvg3AlraLH286n0s376BdxDlHS-ZuBcEHuc5C-rRvT1NaOhW_0c0GkJBTuCCYX8a3GwzwKccf6MZwf7JSDWKDgOi2EzgLREHpWW1rqTwznFTZeMqerX-2z5hIG_8.uA4Qw8oUBu2Fg0rgfFQAlg"
...
ORG_ID = "2746350" # Your ORG_ID, you can find in Apple Search ads, cabinet in the top right menu
...
def main(START_CURRENT, END_CURRENT, START, END):
    ...
    fig = make_subplots( rows=3, cols=2, column_widths=[0.5, 0.5], row_heights=[0.3, .2, 0.5], specs=[[{ "colspan": 2, "secondary_y": True}, None], [{ "colspan": 2, "secondary_y": True}, None], [ { "colspan": 2, "secondary_y": True}   , None]],
                subplot_titles=("ASA KPIs overall", "CPI vs CPT", "ASA CPI per campaign" ))
    ...
    fig.add_trace(go.Scatter(x=custom.date, y=custom.groupby('date').sum()['localSpend']/custom.groupby('date').sum()['impressions'],
                    mode='lines+markers',
    ...
    with open('dash_asa_daily_franki.html', 'w') as f:
        for fig in figs_asa:
...
if __name__ == "__main__":
    current_start= "2021-11-01"
    current_end= "2021-11-16"
    past_start= "2021-10-15"
    past_end= "2021-10-31"
