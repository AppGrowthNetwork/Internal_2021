...
def main():
    ...
    my_app_id = '751912402244298' #'760820158011849'
    my_app_secret = '339cecf7f50c377ac904d10a54ee264a' #'762c8b6bbf2ab8a317f5652d2c2cfc1a'
    my_access_token = "EAAKr3DZBjUsoBAHRWrhH8oqVLRGHH8ZCiipFqse4UGwWgBRJrWzjyvzWJyh8Bu8cvg0oU5ZADAzByKLp0Xzr9RRMUl0L8EdrW0T3IwXNqZBI1WNw6mCbldSX3P4fzzesNZCAaZAxAI7ck7oy2gd7yaHlCc2tEvZBfgx6lpmAHMJHXSqMEHyGeVv"

    FacebookAdsApi.init(my_app_id, my_app_secret, my_access_token)

    sess = AdAccount('act_617639892500962')  #'act_234375110958056'
    ...
    #df.to_csv('fb_db_koc.csv', header = False, mode = 'a') 

    df_cum_total = pd.read_csv('fb_db_koc.csv')
    ...
    app_id = 'com.foundryiv.colorcollab'
    ...
    trace = go.Table(header=dict(values=['Campaign','CTR', 'Conv.Rate']),
              cells=dict(values=[df_camp.campaign_name.values, np.round(df_camp.CTR.values, 2), np.round(df_camp['Conv.Rate'].values, 1)]), domain=dict(x=[0.75, 1],y=[0, .95]))
            
    
    trace0 = go.Pie(values=df_camp.impressions, labels=df_camp.campaign_name, hole=.4, name = 'Impressions', domain=dict(x=[0, 0.18],y=[0, 1]), title_text = 'Impressions', textposition='inside', textinfo='percent', hoverinfo="label+value")
    trace1 = go.Pie(values=df_camp.inline_link_clicks, labels=df_camp.campaign_name, hole=.4, name = 'Clicks', domain=dict(x=[0.19, 0.37],y=[0, 1]), title_text = 'Clicks', textposition='inside', textinfo='percent', hoverinfo="label+value")
    trace2 = go.Pie(values=df_camp.actions, labels=df_camp.campaign_name, hole=.4, name = 'Installs', domain=dict(x=[0.38, 0.56],y=[0, 1]), title_text = 'Installs', textposition='inside', textinfo='percent', hoverinfo="label+value")
    trace3 = go.Pie(values=df_camp.spend, labels=df_camp.campaign_name, hole=.4, name = 'Cost', domain=dict(x=[0.57, .74],y=[0, 1]), title_text = 'Cost', textposition='inside', textinfo='percent', hoverinfo="label+value")
    
   
    
    fig_dough = go.Figure(data = [trace, trace0,trace1, trace2,trace3])
    ...
    with open('Fb_koc_daily.html', 'w') as file:
        for fig in figs_fb:
