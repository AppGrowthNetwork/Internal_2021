...
def main():
    ...
    refresh_token = '1//05n7HZoriFMC0CgYIARAAGAUSNwF-L9IrKzZafavgWApe9b4CzGVwN7DvE6IBLmE6XKROP5N5intXu8uiBTO4UCfr4X20X0oPS-s'
    ...
    df['avgCPC'] = df['Cost'] / df['Conversions']
    temp = df.copy()
    print(temp.head())
    ...
    camp = df.groupby(['Day', 'Campaign']).sum().reset_index()
    ...
    fig = make_subplots( rows=3, cols=2, column_widths=[0.5, 0.5], row_heights=[0.3, .2, 0.5], specs=[[{ "colspan": 2, "secondary_y": True}, None], [{ "colspan": 2, "secondary_y": True}, None], [ { "colspan": 2, "secondary_y": True}   , None]],
                subplot_titles=("Google KPIs overall", "CPC vs CPT", "Google CPC per campaign" ))
    ...
    fig.add_trace(go.Scatter(x=uac_plot.Day, y=uac_plot['Cost']/uac_plot['Impressions'],
                    mode='lines+markers',
                    name='avgCPT'), row=2, col=1)
    for i in df.groupby('Campaign').sum().reset_index()['Campaign'].unique():
        fig.add_trace(go.Scatter(x=uac_plot.Day, y=camp[camp['Campaign'] == i]['avgCPC'],
                   mode='lines+markers', name=i + ' avgCPC'), row=3, col=1)

    fig.add_trace(go.Scatter(x=uac_plot.Day, y=uac_plot['avgCPC'],
                    mode='lines+markers',
                    name='avgCPC', yaxis='y2', marker_color = 'orange'), secondary_y=True, row=1, col=1)
    fig.add_trace(go.Scatter(x=uac_plot.Day, y=uac_plot['avgCPC'],
                    mode='lines+markers',
                    name='avgCPC',  showlegend = False, marker_color = 'orange'),  secondary_y=True, row=2, col=1)

    fig.add_trace(go.Bar(x=uac_plot.Day, y = uac_plot["Impressions"], name='Impressions',
                    marker_color = 'grey'), row= 1, col = 1)

    fig.add_trace(go.Bar(x=uac_plot.Day, y = uac_plot["Clicks"], name='Clicks',
                     marker_color = '#2CA02C'), row= 1, col = 1)

    fig.add_trace(go.Bar(x=uac_plot.Day, y = uac_plot["Conversions"], name='Conversions',
                     marker_color = 'brown'), row= 1, col = 1)
    fig.update_layout(barmode = 'stack')

    #fig.update_yaxes(tickvals=[0, 5, 10, 15, 20, 25, 30, 35, 40, 45], secondary_y=False, row = 2, col = 1)

    figs_gads.append(fig)

    df_camp = camp.groupby('Campaign').sum().reset_index()
    df_camp['CTR'] = df_camp['Clicks'] / df_camp['Impressions']
    df_camp['Conv.Rate'] = df_camp['Conversions'] / df_camp['Clicks']
    df_camp.sort_values('Conv.Rate', ascending = False, inplace = True)
    print(df_camp.head())

    trace0 = go.Pie(values=df_camp.Impressions, labels=df_camp.Campaign, hole=.4, name = 'Impressions', domain=dict(x=[0, 0.24],y=[0, 1]), title_text = 'Impressions', textposition='inside', textinfo='percent', hoverinfo="label+value")
    trace1 = go.Pie(values=df_camp.Clicks, labels=df_camp.Campaign, hole=.4, name = 'Clicks', domain=dict(x=[0.25, 0.49],y=[0, 1]), title_text = 'Clicks', textposition='inside', textinfo='percent', hoverinfo="label+value")
    trace2 = go.Pie(values=df_camp.Conversions, labels=df_camp.Campaign, hole=.4, name = 'Installs', domain=dict(x=[0.50, 0.74],y=[0, 1]), title_text = 'Installs', textposition='inside', textinfo='percent', hoverinfo="label+value")
    trace3 = go.Pie(values=df_camp.Cost, labels=df_camp.Campaign, hole=.4, name = 'Cost', domain=dict(x=[0.75, .99],y=[0, 1]), title_text = 'Cost', textposition='inside', textinfo='percent', hoverinfo="label+value")
    
   
    
    fig_dough = go.Figure(data = [trace0,trace1, trace2,trace3])
    fig_dough.update_layout(title_text="Google Breakdown by Campaign")
    ...
    with open('Adwords_keap_daily.html', 'w') as f:
