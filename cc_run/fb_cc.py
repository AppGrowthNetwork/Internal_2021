...
def main():
    ...
    END_DATE = (datetime.datetime.now() - datetime.timedelta(days = 58)).strftime("%Y-%m-%d")
    #report_id_to_file(START_DATE, END_DATE)

    my_app_id = '760820158011849'
    my_app_secret = '762c8b6bbf2ab8a317f5652d2c2cfc1a'
    my_access_token = "EAAKz9j9BmckBAE8FeQGo6wt68yMXSr5JPIp7ZA7PHO9vGp7hgntu2WUzjYnXF1PiVhKTdZBwUBZAD9dxweZBPk3KkrSrB67QqHcLrWbdLuJ1W1SfdA0fDxiZAcKx4ZBSAk66CZBxbQUACFDAwpGnfBKkFDZAux7sGjmz0YQQGWb0KWu2TbuoJj63"

    FacebookAdsApi.init(my_app_id, my_app_secret, my_access_token)

    sess = AdAccount('act_234375110958056')
    ...
    df_cum_total = pd.read_csv('~/Downloads/cc_run/fb_db_cc.csv')
    #df_cum_total = pd.concat([df_cum_total, df], axis = 0)

    #print(df_cum_total)

    #df.to_csv('fb_db_cc.csv')
    ...
    trace0 = go.Pie(values=df_camp.impressions, labels=df_camp.campaign_name, hole=.4, name = 'Impressions', domain=dict(x=[0, 0.23],y=[0, 1]), title_text = 'Impressions', textposition='inside', textinfo='percent', hoverinfo="label+value")
    trace1 = go.Pie(values=df_camp.inline_link_clicks, labels=df_camp.campaign_name, hole=.4, name = 'Clicks', domain=dict(x=[0.26, 0.49],y=[0, 1]), title_text = 'Clicks', textposition='inside', textinfo='percent', hoverinfo="label+value")
    trace2 = go.Pie(values=df_camp.actions, labels=df_camp.campaign_name, hole=.4, name = 'Installs', domain=dict(x=[0.52, 0.75],y=[0, 1]), title_text = 'Installs', textposition='inside', textinfo='percent', hoverinfo="label+value")
    trace3 = go.Pie(values=df_camp.spend, labels=df_camp.campaign_name, hole=.4, name = 'Cost', domain=dict(x=[0.78, 1],y=[0, 1]), title_text = 'Cost', textposition='inside', textinfo='percent', hoverinfo="label+value")
